...
async def upload_image(file: UploadFile, bookId: int, user: schemas.User = Depends(get_current_user_from_token)):
    try:
...
def get_images(request: Request, bookId: int, user: schemas.User = Depends(get_current_user_from_token)):
    try:
        db = SessionLocal()
...
def delete_image(request: Request, imageId: int, user: schemas.User = Depends(get_current_user_from_token)):
    try:
        if user.isAdmin == True:
            db = SessionLocal()
...
def delete_ebook(request: Request, ebookId: int, user: schemas.User = Depends(get_current_user_from_token)):
    try:
        if user.isAdmin == True:
            db = SessionLocal()
...
async def upload_ebook(file: UploadFile, request: Request, bookId: int, user: schemas.User = Depends(get_current_user_from_token)):
    try:
        if user.isAdmin == True:
